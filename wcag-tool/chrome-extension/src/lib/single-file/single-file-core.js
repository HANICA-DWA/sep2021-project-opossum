...
class Runner {
  constructor(options, root) {
    ...
    this.options.resourceReferrer =
      this.options.passReferrerOnError && matchResourceReferrer && matchResourceReferrer[0]
    ...
    this.options.content =
      this.options.content || (rootDocDefined ? util.serialize(this.options.doc) : null)
    this.onprogress = options.onprogress || (() => {})
  }

  async loadPage() {
    this.onprogress(
      new ProgressEvent(PAGE_LOADING, { pageURL: this.options.url, frame: !this.root })
    )
    await this.processor.loadPage(this.options.content)
    this.onprogress(
      new ProgressEvent(PAGE_LOADED, { pageURL: this.options.url, frame: !this.root })
    )
  }
  ...
  run(onloadListener, options) {
    ...
    return Promise.all(
      resourceURLs.map(async (requestKey) => {
        const [resourceURL, asBinary, expectedType, baseURI, blockMixedContent] =
...
class Processor {
  ...
  async getPageData() {
    ...
    if (this.options.insertSingleFileComment) {
      ...
      let infobarSaveDate = this.options.saveDate
      if (
        firstComment.nodeType == 8 &&
        (firstComment.textContent.includes('--') ||
    ...
    let filename =
      (await ProcessorHelper.evalTemplate(this.options.filenameTemplate, this.options, content)) ||
      ''
    ...
    if (
      ...
      const extension =
        extensionMatch && extensionMatch[0] && extensionMatch[0].length > 1 ? extensionMatch[0] : ''
    ...
    const matchTitle =
      this.baseURI.match(/([^/]*)\/?(\.html?.*)$/) || this.baseURI.match(/\/\/([^/]*)\/?$/)
    const pageData = {
      stats: this.stats.data,
      title:
        this.options.title ||
        (this.baseURI && matchTitle ? matchTitle[1] : url.hostname ? url.hostname : ''),
      filename,
      content,
    }
    ...
    if (this.options.retrieveLinks) {
      pageData.links = Array.from(
        new Set(Array.from(this.doc.links).map((linkElement) => linkElement.href))
      )
    }
    return pageData
  }
  ...
  preProcessPage() {
    if (this.options.win) {
      this.doc.body
        ...
        .forEach(
          (element) =>
            element instanceof this.options.win.HTMLElement && this.doc.head.appendChild(element)
        )
    }
  ...
  removeScripts() {
    ...
    const scriptElements = this.doc.querySelectorAll(
      `script:not([type="application/ld+json"]):not([${SCRIPT_TEMPLATE_SHADOW_ROOT}])`
    )
  ...
  async resolveStylesheetURLs() {
    ...
    async function processElement(
      ...
      if (!matchCharsetEquals(stylesheetContent, options.charset)) {
        options = { ...options, charset: getCharset(stylesheetContent) }
  ...
  async resolveFrameURLs() {
    if (!this.options.saveRawPage) {
      const frameElements = Array.from(
        this.doc.querySelectorAll('iframe, frame, object[type="text/html"][data]')
      )
      await Promise.all(
  ...
  async processScripts() {
    await Promise.all(
      Array.from(this.doc.querySelectorAll('script[src], link[rel=preload][as=script][href]')).map(
        async (element) => {
          let resourceURL
          let scriptSrc
          ...
          if (frameWindowId) {
            ...
            if (frameData) {
              ...
              if (
                ...
                } else if (frameElement.tagName == 'FRAME') {
                  frameElement.setAttribute(
                    'src',
                    `data:text/html,${pageData.content.replace(/%/g, '%25').replace(/#/g, '%23')}`
                  )
                } else {
  ...
  replaceStylesheets() {
    this.doc.querySelectorAll('style').forEach((styleElement) => {
      ...
      if (stylesheetInfo) {
        ...
        const stylesheetContent = cssTree.generate(stylesheetInfo.stylesheet)
        styleElement.textContent = stylesheetContent
  ...
  replaceStyleAttributes() {
    this.doc.querySelectorAll('[style]').forEach((element) => {
      ...
      if (declarations) {
        ...
        const styleContent = cssTree.generate(declarations)
  ...
  async setDocInfo() {
    ...
    this.options.info = {
      description:
        descriptionElement && descriptionElement.content ? descriptionElement.content.trim() : '',
      ...
      publisher:
        publisherElement && publisherElement.content ? publisherElement.content.trim() : '',
      heading:
        headingElement && headingElement.textContent ? headingElement.textContent.trim() : '',
    }
...
class ProcessorHelper {
  static async evalTemplate(template = '', options, content, dontReplaceSlash) {
    ...
    template = await evalTemplateVariable(
      template,
      'url-pathname',
      () => decode(url.pathname).replace(/^\//, '').replace(/\/$/, '') || 'No pathname',
      dontReplaceSlash === undefined ? true : dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
    ...
    template = await evalTemplateVariable(
      template,
      'url-last-segment',
      () => decode(getLastSegment(url, options.filenameReplacementCharacter)) || 'No last segment',
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    if (content) {
  ...
  static setBackgroundImage(element, url, style) {
    ...
    element.style.setProperty(
      'background-position',
      style && style['background-position'] ? style['background-position'] : 'center',
      'important'
    )
    element.style.setProperty(
      'background-color',
      style && style['background-color'] ? style['background-color'] : 'transparent',
      'important'
    )
    element.style.setProperty('background-image', url, 'important')
    element.style.setProperty(
      'background-size',
      style && style['background-size'] ? style['background-size'] : '100% 100%',
      'important'
    )
  ...
  static processShortcutIcons(doc) {
    let shortcutIcon = findShortcutIcon(
      Array.from(doc.querySelectorAll('link[href][rel="icon"], link[href][rel="shortcut icon"]'))
    )
    if (!shortcutIcon) {
  ...
  static async resolveImportURLs(
          ...
          if (!testIgnoredPath(resourceURL) && testValidPath(resourceURL)) {
            ...
            if (testValidURL(resourceURL) && !importedStyleSheets.has(resourceURL)) {
              ...
              if (stylesheetContent.includes(cssImport)) {
                ...
                importedStylesheetContent = await ProcessorHelper.resolveImportURLs(
                ...
                if (
                  (workStylesheet.sheet && workStylesheet.sheet.cssRules.length) ||
    ...
    async function getStylesheetContent(resourceURL) {
      ...
      if (!matchCharsetEquals(content.data, content.charset || options.charset)) {
        options = { ...options, charset: getCharset(content.data) }
        ...
        if (declaration.type == 'Declaration' && declaration.value.children) {
          ...
          await Promise.all(
            urlFunctions.map(async (urlFunction) => {
              ...
              if (!testIgnoredPath(resourceURL)) {
                if (testValidURL(resourceURL)) {
                  const { content, indexResource, duplicate } = await batchRequest.addURL(
                    resourceURL,
                    {
                      asBinary: true,
                      expectedType: 'image',
                      ...
                      if (!originalResourceURL.startsWith('#')) {
                        if (
                          ...
                          const value = cssTree.parse(
                            `var(${SINGLE_FILE_VARIABLE_NAME_PREFIX}${indexResource})`,
                            { context: 'value' }
                          ).children.head
            ...
            if (testValidPath(resourceURL)) {
              ...
              if (testValidURL(resourceURL)) {
                ...
                if (removeElementIfMissing && content == EMPTY_DATA_URI) {
                  ...
                  const forbiddenPrefixFound = PREFIXES_FORBIDDEN_DATA_URI.filter((prefixDataURI) =>
      ...
      Array.from(resourceElements).map(async (resourceElement) => {
        ...
        const srcsetValues = await Promise.all(
          srcset.map(async (srcsetValue) => {
            ...
            return (
              resourceURL +
              (srcsetValue.w ? ` ${srcsetValue.w}w` : srcsetValue.d ? ` ${srcsetValue.d}x` : '')
            )
          })
        )
    ...
    if (attributeValue) {
      ...
      if (imageData && imageData.replaceable) {
        ...
        const backgroundSize =
          (imageData.objectFit == 'content' || imageData.objectFit == 'cover') &&
          imageData.objectFit
        if (backgroundSize) {
...
function findShortcutIcon(shortcutIcons) {
  ...
  shortcutIcons.sort(
    (linkElement1, linkElement2) =>
      (parseInt(linkElement2.sizes, 10) || 16) - (parseInt(linkElement1.sizes, 10) || 16)
  )
...
function removeCssComments(stylesheetContent) {
  try {
    ...
    do {
      ...
      if (start != -1 && end != -1) {
        stylesheetContent =
          stylesheetContent.substring(0, start) + stylesheetContent.substr(end + 2)
      }
    } while (start != -1 && end != -1)
    return stylesheetContent
  }
