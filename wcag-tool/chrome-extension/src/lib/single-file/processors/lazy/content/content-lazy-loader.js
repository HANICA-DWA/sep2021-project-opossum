...
async function process(options) {
  if (document.documentElement) {
    ...
    const maxScrollY = Math.max(
      document.documentElement.scrollHeight - document.documentElement.clientHeight * 1.5,
      0
    )
    const maxScrollX = Math.max(
      document.documentElement.scrollWidth - document.documentElement.clientWidth * 1.5,
      0
    )
        ...
        const updated = mutations.filter((mutation) => {
          ...
          if (
            ...
            return (
              ...
              !mutation.target.classList.contains(helper.SINGLE_FILE_UI_ELEMENT_CLASS)
            )
          }
        })
        if (updated.length) {
