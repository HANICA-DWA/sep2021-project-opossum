...
;((globalThis) => {
  ...
  function loadDeferredImagesStart(keepZoomLevel) {
    ...
    if (!globalThis._singleFileImage) {
      ...
      globalThis.__defineGetter__('Image', () => {
        return function () {
          ...
          result.__defineGetter__('srcset', () => {
            return image.srcset
          })
          image.onload =
            image.onloadend =
    ...
    if (zoomFactor < 1) {
      ...
      document.documentElement.style.setProperty(
        'transform-origin',
        `${zoomFactorX < 1 ? '50%' : '0'} ${zoomFactorY < 1 ? '50%' : '0'} 0`,
        'important'
      )
      document.documentElement.style.setProperty(
        'transform',
      ...
      if (window == window.top) {
        ;[...observers].forEach(([intersectionObserver, observer]) => {
          const getBoundingClientRectDefined =
            observer.options && observer.options.root && observer.options.root.getBoundingClientRect
          const rootBoundingRect =
            getBoundingClientRectDefined && observer.options.root.getBoundingClientRect()
  ...
  addEventListener(BLOCK_COOKIES_START_EVENT, () => {
    try {
      document.__defineGetter__('cookie', () => {
        throw new Error('document.cookie temporary blocked by SingleFile Lite')
      })
    } catch (error) {
      // ignored
    }
  ...
  addEventListener(BLOCK_STORAGE_START_EVENT, () => {
    if (!globalThis._singleFile_localStorage) {
      ...
      globalThis.__defineGetter__('localStorage', () => {
        throw new Error('localStorage temporary blocked by SingleFile Lite')
      })
    }
    if (!globalThis._singleFile_indexedDB) {
      ...
      globalThis.__defineGetter__('indexedDB', () => {
        throw new Error('indexedDB temporary blocked by SingleFile Lite')
      })
    }
  })
  ...
  if (globalThis.FontFace) {
    ...
    globalThis.FontFace = function () {
      if (!warningFontFaceDisplayed) {
        warn(
          'SingleFile Lite is hooking the FontFace constructor, document.fonts.delete and document.fonts.clear to handle dynamically loaded fonts.'
        )
  ...
  if (globalThis.IntersectionObserver) {
    ...
    globalThis.IntersectionObserver = function () {
      if (!warningIntersectionObserverDisplayed) {
        warn(
          'SingleFile Lite is hooking the IntersectionObserver API to detect and load deferred images.'
        )
      ...
      const unobserveIntersection =
        IntersectionObserver.prototype.unobserve || intersectionObserver.unobserve
