...
if (TOP_WINDOW) {
  ...
  if (
    ...
    browser.runtime.onMessage.addListener((message) => {
      ...
      if (message.method == ACK_INIT_REQUEST_MESSAGE) {
...
export { getAsync, getSync, cleanup, initResponse, TIMEOUT_INIT_REQUEST_MESSAGE }
...
function processFramesAsync(doc, frameElements, options, parentWindowId, sessionId) {
  ...
  frameElements.forEach((frameElement, frameIndex) => {
    ...
    requestTimeouts[windowId] = globalThis.setTimeout(
      () => sendInitResponse({ frames: [{ windowId, processed: true }], sessionId }),
...
function sendMessage(targetWindow, message, useChannel) {
  if (targetWindow == top && browser && browser.runtime && browser.runtime.sendMessage) {
    ...
    targetWindow.postMessage(
      MESSAGE_PREFIX + JSON.stringify({ method: message.method, sessionId: message.sessionId }),
      TARGET_ORIGIN,
      [channel.port2]
    )
