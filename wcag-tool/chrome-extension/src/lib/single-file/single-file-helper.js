...
function preProcessDoc(doc, win, options) {
  ...
  doc.querySelectorAll('svg foreignObject').forEach((element) => {
    const flowElements = element.querySelectorAll(
      `html > head > ${FLOW_ELEMENTS_SELECTOR}, html > body > ${FLOW_ELEMENTS_SELECTOR}`
    )
  ...
  elements.forEach((element) => {
    ...
    if (shadowRoot && !element.classList.contains(SINGLE_FILE_UI_ELEMENT_CLASS)) {
      ...
      if (shadowRoot.adoptedStyleSheets && shadowRoot.adoptedStyleSheets.length) {
        shadowRootInfo.adoptedStyleSheets = Array.from(shadowRoot.adoptedStyleSheets).map(
          (stylesheet) =>
  ...
  if (element.tagName == 'IMG') {
    const imageData = {
      currentSrc: elementHidden
        ...
        : (options.loadDeferredImages && element.getAttribute(LAZY_SRC_ATTRIBUTE_NAME)) ||
          element.currentSrc,
    }
      ...
      .forEach((fontFamilyName) => {
        ...
        if (
          ...
          options.loadedFonts.find(
            (font) => normalizeFontFamily(font.family) == fontFamilyName && font.style == fontStyle
          )
        ) {
...
function getSize(win, imageElement, computedStyle) {
  ...
  if (!pxWidth && !pxHeight) {
    ...
    if (computedStyle.getPropertyValue('box-sizing') == 'content-box') {
      ...
      const { clientWidth } = imageElement
    ...
    pxWidth = Math.max(
      0,
      imageElement.clientWidth - paddingLeft - paddingRight - borderLeft - borderRight
    )
    pxHeight = Math.max(
      0,
      imageElement.clientHeight - paddingTop - paddingBottom - borderTop - borderBottom
    )
  }
