  ...
  fontsDetails.fonts.forEach((fontInfo, fontKey) => {
    fontsDetails.fonts.set(
      ...
      fontInfo.map((fontSource) => {
        ...
        let fontFormat
        const urlMatch =
          fontSource.match(REGEXP_URL_SIMPLE_QUOTES_FN) ||
  ...
  for (let cssRule = cssRules.head; cssRule; cssRule = cssRule.next) {
    ...
    if (
      ...
      await processFontFaceRules(
        ruleData.block.children,
        sheetIndex,
        fontsDetails.medias.get(`media-${sheetIndex}-${mediaIndex}-${mediaText}`),
        fontURLs,
        fontTests,
        stats
      )
      ...
      await processFontFaceRules(
        ruleData.block.children,
        sheetIndex,
        fontsDetails.supports.get(`supports-${sheetIndex}-${supportsIndex}-${supportsText}`),
        fontURLs,
        fontTests,
        stats
      )
...
async function processFontFaceRule(ruleData, fontInfo, fontURLs, fontTests, stats) {
  ...
  if (srcDeclaration) {
    await Promise.all(
      fontInfo.map(async (source, sourceIndex) => {
    ...
    const findSource = (fontFormat, testValidity) =>
      fontInfo.find(
        (source) =>
    ...
    const filterSource = (fontSource) =>
      fontInfo.filter((source) => source == fontSource || source.src.startsWith(LOCAL_SOURCE))
    stats.fonts.processed += fontInfo.length
    stats.fonts.discarded += fontInfo.length
    const woffFontFound =
      findSource('woff2-variations', true) || findSource('woff2', true) || findSource('woff', true)
    if (woffFontFound) {
