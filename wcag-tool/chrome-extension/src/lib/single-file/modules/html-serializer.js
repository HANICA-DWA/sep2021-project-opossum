...
const OMITTED_START_TAGS = [
  {
    tagName: 'head',
    accept: (element) =>
      !element.childNodes.length || element.childNodes[0].nodeType == Node_ELEMENT_NODE,
  },
  ...
  { tagName: 'html', accept: (next) => !next || next.nodeType != Node_COMMENT_NODE },
  {
    ...
    accept: (next) =>
      ...
      (next.nodeType != Node_COMMENT_NODE &&
        (next.nodeType != Node_TEXT_NODE || !startsWithSpaceChar(next.textContent))),
  },
  { tagName: 'body', accept: (next) => !next || next.nodeType != Node_COMMENT_NODE },
  {
    ...
    accept: (next, element) =>
      (!next &&
        ...
        (element.parentElement.tagName == 'UL' || element.parentElement.tagName == 'OL')) ||
...
function serializeElement(element, compressHTML, isSVG) {
  ...
  const omittedStartTag =
    ...
    OMITTED_START_TAGS.find(
      (omittedStartTag) => tagName == omittedStartTag.tagName && omittedStartTag.accept(element)
    )
  let content = ''
  if (!omittedStartTag || element.attributes.length) {
    content = `<${tagName}`
    Array.from(element.attributes).forEach(
      (attribute) => (content += serializeAttribute(attribute, element, compressHTML))
    )
    content += '>'
  }
  if (element.tagName == 'TEMPLATE' && !element.childNodes.length) {
    ...
    Array.from(element.childNodes).forEach(
      (childNode) => (content += serialize(childNode, compressHTML, isSVG || tagName == 'svg'))
    )
  }
  const omittedEndTag =
    compressHTML &&
    OMITTED_END_TAGS.find(
      (omittedEndTag) =>
        tagName == omittedEndTag.tagName && omittedEndTag.accept(element.nextSibling, element)
    )
