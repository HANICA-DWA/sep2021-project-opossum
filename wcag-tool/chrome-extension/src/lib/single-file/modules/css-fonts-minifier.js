...
import { normalizeFontFamily, flatten, getFontWeight, removeQuotes } from '../single-file-helper.js'
    ...
    fontsInfo.used = fontsInfo.used.map((fontNames) =>
      fontNames.map((familyName) => {
        ...
        if (matchedVar && matchedVar[1]) {
          ...
          return (
            (computedFamilyName &&
              computedFamilyName.split(',').map((name) => helper.normalizeFontFamily(name))) ||
            familyName
          )
        }
        return familyName
    ...
    fontsInfo.used.forEach((fontNames) =>
      fontNames.forEach((familyName) => {
        if (fontsInfo.declared.find((fontInfo) => fontInfo.fontFamily == familyName)) {
          const optionalData =
            options.usedFonts && options.usedFonts.filter((fontInfo) => fontInfo[0] == familyName)
...
function testFontweight(fontWeight, usedFontWeights) {
  return fontWeight
    .split(',')
    .find((weightValue) =>
      usedFontWeights.includes(helper.getFontWeight(helper.removeQuotes(weightValue)))
    )
}
...
function getUsedFontWeight(fontInfo, fontStyle, fontWeights) {
  ...
  if (fontInfo[2] == fontStyle) {
    const fontWeight = Number(fontInfo[1])
    ...
    const result = unicodeRanges.filter((rangeValue) => {
      ...
      if (range.length == 1) {
        if (range[0].includes('?')) {
          ...
          regExpString = `[${firstRange.replace(REGEXP_QUESTION_MARK, '0')}-${secondRange.replace(
