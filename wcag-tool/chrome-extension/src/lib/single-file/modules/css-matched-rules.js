        ...
        matchedElements = Array.from(doc.querySelectorAll(selectorText)).filter(
          (matchedElement) =>
            ...
            (matchedElement.style.getPropertyValue(DISPLAY_STYLE) != 'none' ||
              matchedElement.style.getPropertyPriority('display') != 'important')
        )
      }
    } catch (error) {
      // ignored
...
function getFilteredSelector(selector, selectorText) {
  ...
  if (removedSelectors.length) {
    removedSelectors.forEach(({ parentSelector, selector }) => {
      if (
        parentSelector.data.children.getSize() == 0 ||
        !selector.prev ||
  ...
  function filterPseudoClasses(selector, parentSelector) {
    ...
    if (
      ...
      (selector.data.type == 'PseudoElementSelector' &&
        (testVendorPseudo(selector) || IGNORED_PSEUDO_ELEMENTS.includes(selector.data.name)))
    ) {
  ...
  function testVendorPseudo(selector) {
    const { name } = selector.data
    return name.startsWith('-') || name.startsWith('\\-')
  }
}

function addRule(element, selectorInfo, styles) {
  const { mediaInfo } = selectorInfo.ruleInfo
...
function computeCascade(mediaInfo, parentMediaInfo, mediaAllInfo, workStylesheet) {
        ...
        if (declarationsInfo.selectorInfo.ruleInfo || mediaInfo == mediaAllInfo) {
          ...
          if (declarationsInfo.selectorInfo.ruleInfo) {
            ...
            const ascendantMedia =
              [mediaInfo, ...parentMediaInfo].find((media) => media.rules.get(ruleData)) ||
              mediaInfo
  ...
  mediaInfo.medias.forEach((childMediaInfo) =>
    computeCascade(childMediaInfo, [mediaInfo, ...parentMediaInfo], mediaAllInfo, workStylesheet)
  )
}
