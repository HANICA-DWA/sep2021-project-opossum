...
let toolbarPositionPointer
let toolbarMoving
let toolbarTranslateMax
...
function toolbarOnTouchMove(event) {
  if (toolbarPositionPointer != null && (event.buttons === undefined || event.buttons == 1)) {
    ...
    let newToolbarTranslate =
      (orientationPortrait ? position.pageY : position.pageX) - toolbarPositionPointer
...
addEventListener('message', (event) => {
  ...
  if (message.method == 'onInit') {
    ...
    if (!message.reset) {
      const { defaultEditorMode } = tabData.options
...
function formatPage() {
  ...
  editorElement.contentWindow.postMessage(
    JSON.stringify({
      method: tabData.options.applySystemTheme ? 'formatPage' : 'formatPageNoTheme',
    }),
    '*'
  )
}
