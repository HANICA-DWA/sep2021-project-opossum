...
let DISABLED_PROFILE_NAME
let CURRENT_PROFILE_NAME
browser.runtime
  .sendMessage({ method: 'config.getConstants' })
  .then((data) => ({ DEFAULT_PROFILE_NAME, DISABLED_PROFILE_NAME, CURRENT_PROFILE_NAME } = data))
...
expandAllButton.addEventListener(
  ...
  () => {
    ...
    document
      ...
      .forEach((detailElement) => (detailElement.open = Boolean(expandAllButton.className)))
  },
  false
)
...
saveToClipboardInput.addEventListener(
  'click',
  () => enableClipboardSave(saveToClipboardInput),
  false
)
...
saveToFilesystemInput.addEventListener(
  'click',
  async () => await browser.runtime.sendMessage({ method: 'downloads.disableGDrive' }),
  false
)
saveToClipboardInput.addEventListener(
  'click',
...
async function init() {
  ...
  loadDeferredImagesMaxIdleTimeLabel.textContent =
    messages.optionLoadDeferredImagesMaxIdleTime.message
  loadDeferredImagesKeepZoomLevelLabel.textContent =
    messages.optionLoadDeferredImagesKeepZoomLevel.message
  ...
  filenameConflictActionUniquifyLabel.textContent =
    messages.optionFilenameConflictActionUniquify.message
  filenameConflictActionOverwriteLabel.textContent =
    messages.optionFilenameConflictActionOverwrite.message
  filenameConflictActionPromptLabel.textContent =
    messages.optionFilenameConflictActionPrompt.message
  ...
  defaultEditorModeCutExternalLabel.textContent =
    messages.optionDefaultEditorModeCutExternal.message
  ...
  ruleUrlInput.placeholder = ruleEditUrlInput.placeholder =
    messages.optionsAutoSettingsUrlPlaceholder.message
  ...
  resetCancelButton.textContent =
    ...
    cancelButton.textContent =
      messages.optionsCancelButton.message
...
async function refresh(profileName) {
  ...
  Array.from(rulesDataElement.childNodes).forEach(
    (node) => (!node.className || !node.className.includes('rule-edit')) && node.remove()
  )
