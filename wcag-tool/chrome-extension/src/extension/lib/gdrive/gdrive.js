...
async function authFromCode(gdrive, options) {
  const httpResponse = await fetch(TOKEN_URL, {
    ...
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body:
      `client_id=${gdrive.clientId}&grant_type=authorization_code` +
      `&code=${options.code}&redirect_uri=${gdrive.redirectURI}`,
  })
  const response = await getJSON(httpResponse)
...
async function initAuth(gdrive, options) {
  ...
  if (options.extractAuthCode) {
    options
      .extractAuthCode(getAuthURL(gdrive, options))
...
function getAuthURL(gdrive, options = {}) {
  ...
  gdrive.authURL =
    `${AUTH_URL}?client_id=${gdrive.clientId}&response_type=code` +
    `&access_type=offline` +
    `&redirect_uri=${gdrive.redirectURI}&scope=${gdrive.scopes.join(' ')}`
...
async function getFolder(gdrive, folderName, parentFolderId) {
  const httpResponse = await fetch(
    `${GDRIVE_URL}?q=mimeType = 'application/vnd.google-apps.folder' and name = '${folderName}' and trashed != true and '${parentFolderId}' in parents`,
    {
      headers: {
...
async function sendFile(mediaUploader) {
  ...
  let end = mediaUploader.file.size
