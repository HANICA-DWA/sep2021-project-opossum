...
function Readability(doc, options) {
  ...
  if (this._debug) {
    const logNode = function (node) {
      ...
      const attrPairs = Array.from(node.attributes || [], (attr) => {
...
Readability.prototype = {
  ...
  _fixRelativeUris(articleContent) {
    ...
    this._forEachNode(medias, function (media) {
      ...
      if (srcset) {
        const newSrcset = srcset.replace(this.REGEXPS.srcsetUrl, (_, p1, p2, p3) => {
  ...
  _getJSONLD(doc) {
    ...
    const jsonLdElement = this._findNode(scripts, (el) => {
