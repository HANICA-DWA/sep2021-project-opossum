...
const Awareness = ({ provider, clientId }) => {
  ...
  const [{}, { anySliderIsOpen, elementSelectorIsOpen }] = useSliders()
  const position = anySliderIsOpen ? 'transform-to-right' : 'transform-to-left'
    ...
    >
      {clients.map((client) => (
        <div
          key={client.id}
          title={client.name}
          className="flex justify-center items-center rounded-full my-1 h-8 w-8 opacity-60 hover:opacity-100"
          style={{ backgroundColor: client.color }}
        >
          <p className="text-center text-sm font-poppins-semi cursor-default">
