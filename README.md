...
2. Run `npm run setup`
3. Run `npm run seed:dev`
4. Run `npm run start`
5. Open a chrome browser and browse to `chrome://extensions/`
6. Make sure developer mode is checked.
7. Click on `Load unpacked`
8. Select the following folder `\wcag-tool\chrome-extension\dist`
9. Make sure your database instance is running if you're using a local Mongo instance.
...
3. **Issue:** Axe Core does not use WCAG 2.1 rules. It uses their own set of rules which is derrived from WCAG and a few other standard, read [this](https://github.com/dequelabs/axe-core/blob/master/doc/rule-descriptions.md) for more  
   **Fix:** To be determined.
4. **Issue:** The alerts after generating annotations with Axe Core will show up after opening and closing the slider even when dismissed earlier.  
   **Fix:** Properly keep track if the alert should be hidden or not with react state.

5. **Issue:** When two or more people are collaborating on an annotation and one of them is on the detail or edit slider of an annotation and another person deletes the annotation, the person viewing the annotating will be in an unwanted state.  
...
6. **Issue:** Lag when scrolling through the list of annotations due to highlighting the element on hover.  
   **Fix:** Make it so that the element only gets highlighted after hovering for over 100-300ms.

7. **Issue:** The WCAG dropdown shows 4 sets of the same rules as they are mapped wrong.  
   **Fix:** Map the WCAG data correctly.

8. **Issue:** When selecting a WCAG, you can't unselect it.  
   **Fix:** Add an option that doesn't represent a WCAG.

9. **Issue:** When return from the detail page of an annotation to the list overview, the scrollbar of list will be at the top instead of the annotation where you scrolled to.  
...
10. **Issue:** When clicking on the 'Delete annotation' button and the tooltip shows up to confirm and you press esc to close the slider and then reopen a detail slider the tooltip will be opened and out of position.  
    **Fix**: Make sure the tooltip is closed when exiting the detail slider

11. **Issue:** No error is displayed to the user when something goes wrong during the following events:
